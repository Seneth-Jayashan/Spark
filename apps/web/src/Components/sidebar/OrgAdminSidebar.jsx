...
import { FaTachometerAlt, FaTasks, FaCalendarAlt, FaUser, FaBars, FaSignOutAlt } from "react-icons/fa";
...
const OrgAdminSidebar = ({ isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    ...
    { name: "Our Events", path: "/dashboard/organizer/event", icon: <FaTasks /> },
    ...
    { name: "Organization", path: "/dashboard/organizer/org/view", icon: <FaTasks /> },
    { name: "Organization Create", path: "/dashboard/organizer/org/create", icon: <FaCalendarAlt /> },
    { name: "Organization Update", path: "/dashboard/organizer/org/update", icon: <FaCalendarAlt /> },
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 mt-4">
        {menuItems.map((item) => {
          const active = location.pathname === item.path; // exact match
          return (
            <NavLink
              key={item.name}
              to={item.path}
